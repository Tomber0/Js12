...
class StoreClass {
	...
	NewContact(){
		this.name=prompt('Введите имя','Store');
		//this.number=prompt('Введите номер','xxx-xx-xx');
		this.email=prompt('Введите email','email@mail.com');	
		this.type=2;
		this.AddBunchOfItems(15);
	}
	CheckStore(){
...
class BankClass{
	...
	NewContact(){
		this.name=prompt('Введите имя','BankName');
		//this.number=prompt('Введите номер','xxx-xx-xx');
		this.email=prompt('Введите email','Bank@mail.com');	
		this.type=3;
	}
}
...
//#SuperNoteBook.push([contacts]);
//#SuperNoteBook.push([Banks]);
//#SuperNoteBook.push([Shops]);
var subchoise = 0;
var choise = 0;
var running = true;
...
while (running){//главный цикл
	...
	switch(choise){
			case 1:
				subchoise = Number(prompt('Ваш выбор:1-контакт 2-магазин 3-банк'));
				switch (subchoise){
				case 1:
				...
				break;
				case 2:
					let sampleShop = new StoreClass ('','');
					sampleShop.NewContact();
					Shops.push(sampleShop);	
					SuperNoteBook.push([Shops]);
				break;
				case 3:
					let sampleBank = new BankClass('','');
					sampleBank.NewContact();
